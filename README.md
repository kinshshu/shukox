...
端口：443
默认UUID：24b4b1e1-xxxx-xxxx-xxxx-242cf53b5bdb
vmess额外id：0
加密：none
传输协议：ws
伪装类型：none
伪装域名：xxx-xxx.koyeb.app
路径：/24b4b1e1-xxxx-xxxx-xxxx-242cf53b5bdb-vless
vless使用(/自定义UUID码-vless)，vmess使用(/自定义UUID码-vmess)
底层传输安全：tls
跳过证书验证：false
```
...
```bash
{
    ...
    "websocket": {
        "enabled": true,
        "path": "/24b4b1e1-xxxx-xxxx-xxxx-242cf53b5bdb-trojan",
...
端口: 443
密码：24b4b1e1-xxxx-xxxx-xxxx-242cf53b5bdb
加密：chacha20-ietf-poly1305
插件程序：xray-plugin_windows_amd64.exe
说明：需将插件 https://github.com/shadowsocks/xray-plugin/releases 下载解压后放至shadowsocks同目录
插件选项: tls;host=xxx-xxx.koyeb.app;path=/24b4b1e1-xxxx-xxxx-xxxx-242cf53b5bdb-ss
```
